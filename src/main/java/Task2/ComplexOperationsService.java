...
public class ComplexOperationsService {
    ...
    public void validateIPAddress(String ipAddress) {
        ...
        // "^([0-9]{1,3}\\.){3}[0-9]{1,3}$" регулярка в тестах отрабтывает не до конца правильно, я исправил на
        // другую регулярку
        if (!ipAddress.matches("^((25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)\\.){3}(25[0-5]|2[0-4]\\d|1\\d\\d|[1-9]?\\d)$")) {
